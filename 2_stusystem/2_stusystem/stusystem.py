...
def show(lst):
    ...
    for item in lst:
       # student_show =dict(eval(item))
        print(format_info.format(item.get('id'),
                                 item.get('name'),
                                 item.get('Python成绩'),
                                 item.get('英语成绩'),
                                 item.get('数学成绩')))
...
def sort():
    ...
    if os.path.exists(file_name):
        ...
        for item in student_list:
            element = dict(eval(item))
            student_info.append(element)
    else:
        return
    acending_or_decending = input('请输入筛选方式（1.升序 2.降序）')
    if acending_or_decending == '1':
        acending_or_decending_bool = False
    elif acending_or_decending == '2':
        acending_or_decending_bool = True
    else:
        print('输入有误，请重新输入')
        sort()
    mode = input('请选择排序方式\n1.按Python成绩排序\n2.按英语成绩排序\n3.按数学成绩排序')
    if mode == '1':
        student_info.sort(key=lambda x:int(x['Python成绩']), reverse=acending_or_decending_bool)
    elif mode == '2':
        student_info.sort(key=lambda x: int(x['英语成绩']), reverse=acending_or_decending_bool)
    elif mode == '3':
        student_info.sort(key=lambda x: int(x['数学成绩']), reverse=acending_or_decending_bool)
    else:
        print('输入有误，请重新输入')
        sort()
    show(student_info)
def show_all():
    if os.path.exists(file_name):
        with open(file_name,'r') as rfile:
            student = rfile.readlines()
            if student:
                format_tile = '{:^6} {:^6} {:^6} {:^6} {:^6}'
                print(format_tile.format('id','学生姓名','Python成绩','英语成绩','数学成绩'))
                format_data = '{:^6} {:^6} {:^14} {:^8} {:^8}'
                for item in student:
                    student_info = dict(eval(item))
                    print(format_data.format(student_info.get('id'),
                                             student_info.get('name'),
                                             student_info.get('Python成绩'),
                                             student_info.get('英语成绩'),
                                             student_info.get('数学成绩')))
            else:
                print('还没有学生信息，请先录入')
    else:
        print('未找到学生文件')
